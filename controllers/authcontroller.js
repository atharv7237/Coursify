...
module.exports.handleGenerateOtp = async (req, res) => {
  ...
  try {
    ...
    const sendmail = await sendEmail(user.Email, subject, message);
    const mail = await jwt.sign({ email: user.Email }, process.env.Jwt);
...
module.exports.handleVerifyOtp = async (req, res) => {
  let { otp } = req.body;
  let otp1 = Number(otp);
  let Email = await jwt.verify(req.cookies.email, process.env.Jwt).email;
  console.log(Email);
  try {
    const otpRecord = await Otp.findOne({ email: Email, otp: otp1 });
    if (
      !otpRecord ||
    ...
    let token = await jwt.sign({ email: Email }, process.env.Jwt);
...
module.exports.handleResetPassword = async (req, res) => {
  try {
    ...
    let Email = await jwt.verify(req.cookies.verify, process.env.Jwt).email;
    console.log(Email);
